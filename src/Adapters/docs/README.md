...
`BunkFy.AdapterHost` provides the runnable single-connection daemon. Its preferred `server-lease` mode uses the Ingestion-owned checkpoint and fencing contract; the Development-compatible `local-file` mode holds an exclusive lock on a versioned atomic checkpoint file. It reloads bounded configuration/secret files each cycle and runs one cycle at a time with timeout and bounded retry. `/health/live` and `/health/ready` are payload-minimal; `/status` is disabled or loopback-only in Production and never includes tenant, property, checkpoint text, paths, token, material, or payloads. Readiness requires runner selection plus readable token and current material, and local-file mode also requires its checkpoint lease. `fake.http`, `json.file-drop`, and `imap.reservation-json` descriptors support both BunkFy-owned polling and standalone push delivery.
