...
internal sealed partial class InventoryTenantTerminationContributor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return Failed(
                "inventory.termination.destroy-request-invalid",
                startedAtUtc);
        }
        ...
        {
            return Failed(
                "inventory.termination.destroy-transaction-conflict",
                startedAtUtc);
        }
        ...
        try
        {
            if (dbContext.Database.IsRelational())
            {
                ...
                await InventoryTenantMutationLock.AcquireLifecycleAsync(
                        dbContext,
                        tenantId,
            ...
            if (!Matches(request, fence))
            {
                return await FinishAsync(
                    transaction,
                    RetryRequired(
                        "inventory.termination.destroy-fence-unavailable",
                        clock.UtcNow),
                    cancellationToken).ConfigureAwait(false);
            }

            InventoryTenantDestroyReceipt[] receiptMatches =
            ...
            {
                InventoryTenantDestroyReceipt? exact = receiptMatches
            ...
            InventoryTenantDestroyOperation[] operationMatches =
                await dbContext.TenantDestroyOperations
                    ...
                    .ToArrayAsync(cancellationToken)
                    .ConfigureAwait(false);
            InventoryTenantDestroyOperation? operation = operationMatches
            ...
            InventoryTenantRevision? state = await dbContext.TenantRevisions
            ...
            if (operation is null)
            {
                ...
                operation = InventoryTenantDestroyOperation.TryCreate(
                    request.IdempotencyKey,
                    tenantId,
                    requestSha256,
                    selectedRevision,
                ...
                if (state is null)
                {
                    state = InventoryTenantRevision.TryBeginClosing(
                        tenantId,
                        request.IdempotencyKey,
                        requestSha256,
                        operationStartedAtUtc);
                ...
                if (operation is null || state is null)
                {
                    return await FinishAsync(
                        transaction,
                        Failed(
                            "inventory.termination.destroy-state-invalid",
                            clock.UtcNow),
                        cancellationToken).ConfigureAwait(false);
                }
                ...
                state.LifecycleStatus != InventoryTenantLifecycleStatus.Closing ||
                    ...
                    cancellationToken).ConfigureAwait(false))
            {
                throw new InvalidDataException(
                    "Inventory tenant destruction completed with remaining owner records.");
            }
            ...
            if (completedAtUtc > request.DeadlineUtc)
            {
                return await FinishAsync(
                    transaction,
                    RetryRequired(
                        "inventory.termination.destroy-deadline-expired",
                        operation.RemovedRecordCount,
                        completedAtUtc),
                    cancellationToken).ConfigureAwait(false);
            }
            ...
            InventoryTenantDestroyReceipt? receipt =
                InventoryTenantDestroyReceipt.TryCreate(
                    operation,
                    completedAtUtc);
            if (receipt is null || !state.CompleteDestruction(completedAtUtc))
            {
                throw new InvalidDataException(
                    "Inventory tenant destruction completion proof is invalid.");
            }
        ...
        string tenantId)
    {
        ...
        return InventoryTenantLifecycleHashes.Sha256(
            "bunkfy-inventory-tenant-destroy-request/v1|" +
