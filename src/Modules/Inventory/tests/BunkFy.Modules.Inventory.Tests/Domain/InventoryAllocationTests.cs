...
public sealed class InventoryAllocationTests
{
    ...
    [Fact]
    public void Amendment_is_versioned_atomic_and_retains_the_active_allocation_identity()
    {
        InventoryAllocation allocation = CreateAccepted([UnitOne]).Value;

        Result<InventoryAllocationAmendmentOutcome> changed = allocation.Amend(
            Guid.NewGuid(),
            expectedVersion: 1,
        ...
        Assert.Equal(InventoryAllocationAmendmentOutcome.Changed, changed.Value);
        Assert.Equal(2, allocation.Version);
        Assert.Equal(InventoryAllocationState.Active, allocation.Status);
        Assert.Equal(new DateOnly(2026, 8, 2), allocation.Arrival);
        Assert.Equal(UnitTwo, Assert.Single(allocation.Units).InventoryUnitId);
        Assert.Equal(
            InventoryDomainErrors.VersionConflict,
            allocation.Amend(Guid.NewGuid(), 1, allocation.Arrival, allocation.Departure, [UnitTwo]).Error);
    }
