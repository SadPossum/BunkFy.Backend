...
public sealed class BedRetirementProcess : ScopedAggregateRoot<Guid>
{
    public const int ReasonMaxLength = 500;
    public const int ActorIdMaxLength = 200;

    private BedRetirementProcess() { }
    ...
    public static Result<BedRetirementProcess> Create(
        Guid id,
        string scopeId,
        Guid propertyId,
        Guid roomId,
        ...
        DateTimeOffset nowUtc)
    {
        ...
        if (id == Guid.Empty || propertyId == Guid.Empty || roomId == Guid.Empty || bedId == Guid.Empty)
        {
            return Result.Failure<BedRetirementProcess>(InventoryDomainErrors.BedRetirementIdentityInvalid);
        }
        ...
        {
            return Result.Failure<BedRetirementProcess>(InventoryDomainErrors.BedRetirementRequestInvalid);
        }

        return Result.Success(new BedRetirementProcess(
            id,
            scopeId.Trim(),
            propertyId,
            roomId,
    ...
    public Result RequestFinalization(Guid eventId, DateTimeOffset nowUtc)
    {
        ...
        if (this.State is not (InventoryRetirementProcessState.Draining or InventoryRetirementProcessState.Rejected) ||
            eventId == Guid.Empty)
        {
            return Result.Failure(InventoryDomainErrors.BedRetirementTransitionInvalid);
        }
        ...
        this.RaiseDomainEvent(new BedRetirementFinalizationRequestedDomainEvent(
            eventId,
            nowUtc,
            this.ScopeId,
            this.Id,
        ...
        DateTimeOffset nowUtc)
    {
            ...
            normalizedActor.Length is 0 or > ActorIdMaxLength)
        {
            return Result.Failure(
                InventoryDomainErrors.BedRetirementCancellationRequestInvalid);
        }
