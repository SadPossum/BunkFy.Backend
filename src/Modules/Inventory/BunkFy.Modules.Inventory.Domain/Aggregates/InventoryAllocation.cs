...
public sealed class InventoryAllocation : ScopedAggregateRoot<Guid>
{
    ...
    public Result<InventoryAllocationAmendmentOutcome> Amend(
        ...
        IReadOnlyCollection<Guid> inventoryUnitIds)
    {
        if (amendmentRequestId == Guid.Empty)
        {
            return Result.Failure<InventoryAllocationAmendmentOutcome>(
                InventoryDomainErrors.AllocationAmendmentRequestIdRequired);
        }

        if (this.Status != InventoryAllocationState.Active)
        {
            return Result.Failure<InventoryAllocationAmendmentOutcome>(InventoryDomainErrors.AllocationNotActive);
        }

        if (expectedVersion != this.Version)
        {
            return Result.Failure<InventoryAllocationAmendmentOutcome>(InventoryDomainErrors.VersionConflict);
        }

        if (arrival >= departure)
        {
            return Result.Failure<InventoryAllocationAmendmentOutcome>(InventoryDomainErrors.StayRangeInvalid);
        }

        ArgumentNullException.ThrowIfNull(inventoryUnitIds);
        Guid[] units = inventoryUnitIds.ToArray();
        if (units.Length is 0 or > MaximumUnits || units.Any(id => id == Guid.Empty) ||
            units.Distinct().Count() != units.Length)
        {
            return Result.Failure<InventoryAllocationAmendmentOutcome>(InventoryDomainErrors.AllocationUnitsInvalid);
        }

        bool sameUnits = this.units.Select(unit => unit.InventoryUnitId).Order()
            .SequenceEqual(units.Order());
        if (this.Arrival == arrival && this.Departure == departure && sameUnits)
        {
            return Result.Success(InventoryAllocationAmendmentOutcome.Unchanged);
        }

        this.Arrival = arrival;
        this.Departure = departure;
        this.units.Clear();
        this.units.AddRange(units.Select(unitId => new InventoryAllocationUnit(unitId, this.ScopeId, this.Id)));
        this.Version++;
        return Result.Success(InventoryAllocationAmendmentOutcome.Changed);
    }
