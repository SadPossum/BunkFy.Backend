...
internal sealed partial class StaffTenantTerminationContributor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return Failed(
                "staff.termination.destroy-request-invalid",
                startedAtUtc);
        }
        ...
        {
            return Failed(
                "staff.termination.destroy-transaction-conflict",
                startedAtUtc);
        }
        ...
        try
        {
            if (dbContext.Database.IsRelational())
            {
                ...
                await StaffTenantMutationLock.AcquireLifecycleAsync(
                        dbContext,
                        tenantId,
            ...
            if (!Matches(request, fence))
            {
                return await FinishAsync(
                    transaction,
                    RetryRequired(
                        "staff.termination.destroy-fence-unavailable",
                        clock.UtcNow),
                    cancellationToken).ConfigureAwait(false);
            }

            StaffTenantDestroyReceipt[] receiptMatches =
            ...
            {
                StaffTenantDestroyReceipt? exact = receiptMatches
            ...
            StaffTenantDestroyOperation[] operationMatches =
                await dbContext.TenantDestroyOperations
                    ...
                    .ToArrayAsync(cancellationToken)
                    .ConfigureAwait(false);
            StaffTenantDestroyOperation? operation = operationMatches
            ...
            StaffTenantRevision? state = await dbContext.TenantRevisions
            ...
            if (operation is null)
            {
                ...
                long activeHoldCount = await dbContext.DataHolds.LongCountAsync(
                        hold => hold.State == StaffDataHoldState.Active,
                        cancellationToken)
                    .ConfigureAwait(false);
                ...
                operation = StaffTenantDestroyOperation.TryCreate(
                    request.IdempotencyKey,
                    tenantId,
                    requestSha256,
                    selectedRevision,
                ...
                if (state is null)
                {
                    state = StaffTenantRevision.TryBeginClosing(
                        tenantId,
                        request.IdempotencyKey,
                        requestSha256,
                        operationStartedAtUtc);
                ...
                if (operation is null || state is null)
                {
                    return await FinishAsync(
                        transaction,
                        Failed(
                            "staff.termination.destroy-state-invalid",
                            clock.UtcNow),
                        cancellationToken).ConfigureAwait(false);
                }
                ...
                state.LifecycleStatus != StaffTenantLifecycleStatus.Closing ||
                    ...
                    cancellationToken).ConfigureAwait(false))
            {
                throw new InvalidDataException(
                    "Staff tenant destruction completed with remaining owner records.");
            }
            ...
            if (completedAtUtc > request.DeadlineUtc)
            {
                return await FinishAsync(
                    transaction,
                    RetryRequired(
                        "staff.termination.destroy-deadline-expired",
                        operation.RemovedRecordCount,
                        completedAtUtc),
                    cancellationToken).ConfigureAwait(false);
            }
            ...
            StaffTenantDestroyReceipt? receipt =
                StaffTenantDestroyReceipt.TryCreate(
                    operation,
                    completedAtUtc);
            if (receipt is null || !state.CompleteDestruction(completedAtUtc))
            {
                throw new InvalidDataException(
                    "Staff tenant destruction completion proof is invalid.");
            }
        ...
        string tenantId)
    {
        ...
        return StaffTenantLifecycleHashes.Sha256(
            "bunkfy-staff-tenant-destroy-request/v1|" +
