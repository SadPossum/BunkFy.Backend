      ...
      ITenantTerminationExportContributor
{
    ...
    public DataRightsExportDescriptor ExportDescriptor =>
        StaffTenantTerminationExportSchema.Descriptor;
    ...
    public Task<TenantTerminationContributionResult> ExecuteAsync(
        ...
        request?.Phase == TenantTerminationContributionPhase.Destroy
            ...
            : Task.FromResult(Failed(
                "staff.termination.export-requires-fragment-assembler",
                clock.UtcNow));
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return Failed(
                "staff.termination.export-request-invalid",
                startedAtUtc);
        }
        ...
        {
            return Failed(
                "staff.termination.export-transaction-conflict",
                startedAtUtc);
        }
        ...
        try
        {
            if (dbContext.Database.IsRelational())
            {
                ...
                await StaffTenantMutationLock.AcquireExclusiveAsync(
                    dbContext,
                    tenantId!,
            ...
            {
                return RetryRequired(
                    "staff.termination.export-revision-changed",
                    clock.UtcNow);
            }
            ...
            {
                return RetryRequired(
                    "staff.termination.export-deadline-expired",
                    completedAtUtc);
            }
    ...
    private async Task<long> GetOrCreateRevisionAsync(
        ...
        CancellationToken cancellationToken)
    {
        StaffTenantRevision? revision = await dbContext.TenantRevisions
            .SingleOrDefaultAsync(cancellationToken)
            .ConfigureAwait(false);
        if (revision is null)
        {
            revision = StaffTenantRevision.Create(tenantId);
