...
internal sealed partial class PropertiesTenantTerminationContributor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return Failed(
                "properties.termination.destroy-request-invalid",
                startedAtUtc);
        }
        ...
        {
            return Failed(
                "properties.termination.destroy-transaction-conflict",
                startedAtUtc);
        }
        ...
        try
        {
            if (dbContext.Database.IsRelational())
            {
                ...
                await PropertiesTenantMutationLock.AcquireLifecycleAsync(
                        dbContext,
                        tenantId,
            ...
            if (!Matches(request, fence))
            {
                return await FinishAsync(
                    transaction,
                    RetryRequired(
                        "properties.termination.destroy-fence-unavailable",
                        clock.UtcNow),
                    cancellationToken).ConfigureAwait(false);
            }

            PropertiesTenantDestroyReceipt[] receiptMatches =
            ...
            {
                PropertiesTenantDestroyReceipt? exact = receiptMatches
            ...
            PropertiesTenantDestroyOperation[] operationMatches =
                await dbContext.TenantDestroyOperations
                    ...
                    .ToArrayAsync(cancellationToken)
                    .ConfigureAwait(false);
            PropertiesTenantDestroyOperation? operation = operationMatches
            ...
            PropertiesTenantRevision? state = await dbContext.TenantRevisions
            ...
            if (operation is null)
            {
                ...
                operation = PropertiesTenantDestroyOperation.TryCreate(
                    request.IdempotencyKey,
                    tenantId,
                    requestSha256,
                    selectedRevision,
                ...
                if (state is null)
                {
                    state = PropertiesTenantRevision.TryBeginClosing(
                        tenantId,
                        request.IdempotencyKey,
                        requestSha256,
                        operationStartedAtUtc);
                ...
                if (operation is null || state is null)
                {
                    return await FinishAsync(
                        transaction,
                        Failed(
                            "properties.termination.destroy-state-invalid",
                            clock.UtcNow),
                        cancellationToken).ConfigureAwait(false);
                }
                ...
                state.LifecycleStatus != PropertiesTenantLifecycleStatus.Closing ||
                    ...
                    cancellationToken).ConfigureAwait(false))
            {
                throw new InvalidDataException(
                    "Properties tenant destruction completed with remaining owner records.");
            }
            ...
            if (completedAtUtc > request.DeadlineUtc)
            {
                return await FinishAsync(
                    transaction,
                    RetryRequired(
                        "properties.termination.destroy-deadline-expired",
                        operation.RemovedRecordCount,
                        completedAtUtc),
                    cancellationToken).ConfigureAwait(false);
            }
            ...
            PropertiesTenantDestroyReceipt? receipt =
                PropertiesTenantDestroyReceipt.TryCreate(
                    operation,
                    completedAtUtc);
            if (receipt is null || !state.CompleteDestruction(completedAtUtc))
            {
                throw new InvalidDataException(
                    "Properties tenant destruction completion proof is invalid.");
            }
        ...
        string tenantId)
    {
        ...
        return PropertiesTenantLifecycleHashes.Sha256(
            "bunkfy-properties-tenant-destroy-request/v1|" +
    ...
    private static TenantTerminationContributionResult Completed(
        ...
        new(
            ...
            PropertiesTenantTerminationMetadata.CatalogSha256,
