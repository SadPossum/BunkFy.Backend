...
internal static class GuestAnonymisationPolicyEvidence
{
    public static GuestAnonymisationRoutingPolicyEvidence FromApproval(
    ...
    public static bool IsValid(GuestAnonymisationRoutingPolicyEvidence? evidence)
    {
        if (evidence is null)
        {
            return false;
    ...
    public static bool MatchesApproval(
        GuestAnonymisationRoutingPolicyEvidence requested,
    ...
    public static string ComputeSha256(GuestAnonymisationRoutingPolicyEvidence evidence)
    {
        ArgumentNullException.ThrowIfNull(evidence);
        StringBuilder canonical = new();
        Append(canonical, evidence.PropertyPolicySourceVersion.ToString(CultureInfo.InvariantCulture));
        Append(canonical, NormalizeUpper(evidence.OperatingCountryCode));
        Append(canonical, NormalizeLower(evidence.PolicyId));
        Append(canonical, evidence.PolicyVersion.ToString(CultureInfo.InvariantCulture));
        Append(canonical, NormalizeLower(evidence.RetentionPolicyId));
        Append(canonical, evidence.RetentionPolicyVersion.ToString(CultureInfo.InvariantCulture));
        ...
        Append(
            canonical,
            evidence.EvaluatedAtUtc.ToUniversalTime().ToString("O", CultureInfo.InvariantCulture));
