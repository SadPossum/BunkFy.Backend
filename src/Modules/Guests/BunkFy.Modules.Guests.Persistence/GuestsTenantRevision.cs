...
internal sealed class GuestsTenantRevision : IScopedEntity
{
    private GuestsTenantRevision()
    {
    }

    private GuestsTenantRevision(string scopeId)
    {
        this.ScopeId = scopeId;
        this.Revision = 1;
        this.LifecycleStatus = GuestsTenantLifecycleStatus.Open;
    }
    ...
    public GuestsTenantLifecycleStatus LifecycleStatus { get; private set; }
    ...
    public static GuestsTenantRevision Create(string tenantId)
    {
        ...
        return new GuestsTenantRevision(scopeId);
    }
    ...
    public void Advance()
    {
        if (!this.IsOpen)
        {
            throw new InvalidOperationException(
                "A closing Guests tenant cannot advance its revision.");
        }

        this.Revision = checked(this.Revision + 1);
    }

    public static GuestsTenantRevision? TryBeginClosing(
        string tenantId,
        Guid operationId,
        string requestSha256,
        DateTimeOffset startedAtUtc)
    {
        GuestsTenantRevision revision = Create(tenantId);
        ...
        revision.LifecycleStatus = GuestsTenantLifecycleStatus.Closing;
        ...
        DateTimeOffset startedAtUtc)
    {
        ...
        this.LifecycleStatus = GuestsTenantLifecycleStatus.Closing;
    ...
    {
        if (this.LifecycleStatus != GuestsTenantLifecycleStatus.Closing ||
        ...
        this.LifecycleStatus = GuestsTenantLifecycleStatus.Closed;
        this.DestroyCompletedAtUtc = completedAtUtc;
        return true;
    }
}

internal enum GuestsTenantLifecycleStatus
{
    Unknown = 0,
    Open = 1,
    Closing = 2,
