      ...
      ITenantTerminationExportContributor
{
    ...
    public Task<TenantTerminationContributionResult> ExecuteAsync(
        ...
        request?.Phase == TenantTerminationContributionPhase.Destroy
            ...
            : Task.FromResult(Failed(
                "guests.termination.export-requires-fragment-assembler",
                clock.UtcNow));
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return Failed(
                "guests.termination.export-request-invalid",
                startedAtUtc);
        }
        ...
        {
            return Failed(
                "guests.termination.export-transaction-conflict",
                startedAtUtc);
        }
        ...
        try
        {
            if (dbContext.Database.IsRelational())
            {
                ...
                await GuestsTenantMutationLock.AcquireExclusiveAsync(
                    dbContext,
                    tenantId!,
            ...
            {
                return RetryRequired(
                    "guests.termination.export-revision-changed",
                    clock.UtcNow);
            }
            ...
            {
                return RetryRequired(
                    "guests.termination.export-deadline-expired",
                    completedAtUtc);
            }
    ...
    private async Task<long> GetOrCreateRevisionAsync(
        ...
        CancellationToken cancellationToken)
    {
        GuestsTenantRevision? revision = await dbContext.TenantRevisions
            .SingleOrDefaultAsync(cancellationToken)
            .ConfigureAwait(false);
        if (revision is null)
        {
            revision = GuestsTenantRevision.Create(tenantId);
