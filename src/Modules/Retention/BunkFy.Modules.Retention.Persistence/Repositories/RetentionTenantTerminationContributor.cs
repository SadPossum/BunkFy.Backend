      ...
      ITenantTerminationExportContributor
{
    ...
    public DataRightsExportDescriptor ExportDescriptor =>
        RetentionTenantTerminationExportSchema.Descriptor;
    ...
    public Task<TenantTerminationContributionResult> ExecuteAsync(
        ...
        request?.Phase == TenantTerminationContributionPhase.Destroy
            ...
            : Task.FromResult(Failed(
                "retention.termination.export-requires-fragment-assembler",
                clock.UtcNow));
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return Failed(
                "retention.termination.export-request-invalid",
                startedAtUtc);
        }
        ...
        {
            return Failed(
                "retention.termination.export-transaction-conflict",
                startedAtUtc);
        }
        ...
        try
        {
            if (dbContext.Database.IsRelational())
            {
                ...
                await RetentionTenantMutationLock.AcquireExclusiveAsync(
                    dbContext,
                    tenantId!,
            ...
            {
                return RetryRequired(
                    "retention.termination.export-revision-changed",
                    clock.UtcNow);
            }
            ...
            {
                return RetryRequired(
                    "retention.termination.export-deadline-expired",
                    completedAtUtc);
            }
            ...
            return new TenantTerminationContributionResult(
                ...
                "retention.termination.exported",
                recordCount,
    ...
    private async Task<long> GetOrCreateRevisionAsync(
        ...
        CancellationToken cancellationToken)
    {
        RetentionTenantRevision? revision = await dbContext.TenantRevisions
            .SingleOrDefaultAsync(cancellationToken)
            .ConfigureAwait(false);
        if (revision is null)
        {
            revision = RetentionTenantRevision.Create(tenantId);
