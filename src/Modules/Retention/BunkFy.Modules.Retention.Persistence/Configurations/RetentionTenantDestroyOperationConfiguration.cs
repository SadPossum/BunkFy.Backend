...
internal sealed class RetentionTenantDestroyOperationConfiguration
    : IEntityTypeConfiguration<RetentionTenantDestroyOperation>
{
    public void Configure(
        EntityTypeBuilder<RetentionTenantDestroyOperation> builder)
    {
        builder.ToTable("tenant_destroy_operations", table =>
        {
            table.HasCheckConstraint(
                "CK_retention_tenant_destroy_operation_revisions",
            ...
            table.HasCheckConstraint(
                "CK_retention_tenant_destroy_operation_progress",
                $"\"Stage\" BETWEEN 1 AND " +
                $"{(int)RetentionTenantDestroyStage.Completed} AND " +
                "\"RemovedRecordCount\" >= 0 AND " +
                "\"CompletedBatchCount\" >= 0 AND " +
                "\"ProofVersion\" = 1 AND \"ConcurrencyVersion\" >= 1");
            table.HasCheckConstraint(
                "CK_retention_tenant_destroy_operation_times",
