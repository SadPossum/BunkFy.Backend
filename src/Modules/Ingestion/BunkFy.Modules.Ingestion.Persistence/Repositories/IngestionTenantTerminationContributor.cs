...
internal sealed partial class IngestionTenantTerminationContributor(
    ...
    IRawPayloadStore? rawPayloads = null)
    : ITenantTerminationContributor,
      ITenantTerminationExportContributor
{
    private const int LargeTextChunkBytes = 12_000;
    private static readonly Guid IngressControlRecordNamespaceId =
        Guid.Parse("3d0f8ee7-cb91-4f3b-a4a8-b3d540e716c7");
    ...
    public DataRightsExportDescriptor ExportDescriptor =>
        IngestionTenantTerminationExportSchema.Descriptor;
    ...
    public Task<TenantTerminationContributionResult> ExecuteAsync(
        ...
        request?.Phase == TenantTerminationContributionPhase.Destroy
            ...
            : Task.FromResult(Failed(
                "ingestion.termination.export-requires-fragment-assembler",
                clock.UtcNow));
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return Failed(
                "ingestion.termination.export-request-invalid",
                startedAtUtc);
        }
        ...
        {
            return Failed(
                "ingestion.termination.export-transaction-conflict",
                startedAtUtc);
        }
        ...
        try
        {
            if (dbContext.Database.IsRelational())
            {
                ...
                await IngestionTenantMutationLock.AcquireExclusiveAsync(
                    dbContext,
                    tenantId!,
            ...
            if (await dbContext.ObservationReceipts
                .AsNoTracking()
                .AnyAsync(
                    receipt => receipt.RawPayloadRetentionState ==
            ...
            {
                return RetryRequired(
                    "ingestion.termination.export-revision-changed",
                    clock.UtcNow);
            }
            ...
            {
                return RetryRequired(
                    "ingestion.termination.export-deadline-expired",
                    completedAtUtc);
            }
            ...
            return new TenantTerminationContributionResult(
                ...
                "ingestion.termination.exported",
                recordCount,
    ...
    private async Task<long> GetOrCreateRevisionAsync(
        ...
        CancellationToken cancellationToken)
    {
        IngestionTenantRevision? revision = await dbContext.TenantRevisions
            .SingleOrDefaultAsync(cancellationToken)
            .ConfigureAwait(false);
        if (revision is null)
        {
            revision = IngestionTenantRevision.Create(tenantId);
