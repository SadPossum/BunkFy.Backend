...
internal sealed class ReservationGuestLinkedOutboxProjector(IOutboxWriterRegistry outboxWriters)
    : IDomainEventHandler<ReservationGuestLinkedDomainEvent>
{
    public Task HandleAsync(ReservationGuestLinkedDomainEvent domainEvent, CancellationToken cancellationToken) =>
        outboxWriters.GetRequired(ReservationsModuleMetadata.Name).EnqueueAsync(
            new ReservationGuestLinkedIntegrationEvent(
                domainEvent.EventId,
                domainEvent.ScopeId,
