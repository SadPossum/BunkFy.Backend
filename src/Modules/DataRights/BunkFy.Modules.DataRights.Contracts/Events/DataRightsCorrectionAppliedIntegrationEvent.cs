...
[TenantScoped]
public sealed record DataRightsCorrectionAppliedIntegrationEvent
    : TenantIntegrationEvent
{
    public const string EventType = "data-rights-correction-applied";
    public const int EventVersion = 1;

    public DataRightsCorrectionAppliedIntegrationEvent(
        Guid eventId,
        string tenantId,
        ...
        DataRightsCorrectionAppliedIntegrationEvent integrationEvent)
    {
        ...
        using (Utf8JsonWriter writer = new(buffer))
        {
            ...
            writer.WriteNumber("contractVersion", DataRightsCorrectionContract.CurrentVersion);
            writer.WriteString("tenantId", integrationEvent.TenantId);
            writer.WriteString("executionId", integrationEvent.ExecutionId);
            writer.WriteString("propertyId", integrationEvent.PropertyId);
            ...
            writer.WriteNumber("currentRecordVersion", integrationEvent.CurrentRecordVersion);
