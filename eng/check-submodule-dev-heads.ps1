...
foreach ($pathRow in $pathRows) {
    ...
    if (-not (Test-GmaGitSuccess -Arguments @('diff', '--quiet', '--', $relativePath))) {
        $errors.Add("Submodule gitlink '$relativePath' has unstaged changes in the repository. Commit the updated pointer or reset the submodule.")
    }

    if (-not (Test-GmaGitSuccess -Arguments @('diff', '--cached', '--quiet', '--', $relativePath))) {
        $errors.Add("Submodule gitlink '$relativePath' has staged but uncommitted changes in the repository.")
    }
    ...
    $recordedCommit = @(Invoke-GmaGitOutput -Arguments @('rev-parse', ":$relativePath"))[0].Trim()
    ...
    if (-not [string]::Equals($recordedCommit, $remoteCommit, [System.StringComparison]::Ordinal)) {
        $errors.Add("Repository records '$relativePath' at $($recordedCommit.Substring(0, 7)), but origin/$Branch is $($remoteCommit.Substring(0, 7)). Commit the latest submodule pointer.")
    }
